# Qanto Testnet Architecture (Zero-Budget Specification)

This document specifies a sustainable, production-grade infrastructure architecture for the Qanto Genesis Testnet operating under a strict **$0/month budget constraint**. It leverages hosting, serverless databases, static front-ends, and cloud object storage, utilizing WebSockets for peer-to-peer network discovery.

---

## 1. Infrastructure Inventory

We will utilize the following permanently:
...
*   **Hosting**: Render or Hugging Face Spaces (Docker space running Node.js express server).
*   **Database**: Neon.tech or Supabase Postgres database (via `DATABASE_URL` environment variable).
