...
impl CudaGpuContext {
    ...
    pub fn hash_batch(
        ...
        self.device
            .dtoh_sync_copy_into(&result_found_gpu, &mut result_found)?;

        if result_found[0] != 0 {
            self.device
                .dtoh_sync_copy_into(&result_nonce_gpu, &mut result_nonce)?;
            self.device
                .dtoh_sync_copy_into(&result_hash_gpu, &mut result_hash)?;
...
lazy_static! {
    pub static ref CUDA_GPU_CONTEXT: Mutex<Option<CudaGpuContext>> = {
        match CudaGpuContext::new() {
            ...
            Err(e) => {
                warn!(
                    "[Qanhash-CUDA] Failed to initialize CUDA GPU context: {}",
                    e
                );
                Mutex::new(None)
            }
