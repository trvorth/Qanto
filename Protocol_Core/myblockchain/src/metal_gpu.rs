use crate::qanhash::{get_qdag, Target, MIX_BYTES};
...
impl MetalGpuContext {
    pub fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let device = Device::system_default().ok_or("No Metal-compatible device found")?;
    ...
    pub fn hash_batch(
        ...
        encoder.set_bytes(
            1,
            8,
            &start_nonce.to_le_bytes() as *const _ as *const std::ffi::c_void,
        );
        encoder.set_buffer(2, Some(&dag_buffer), 0);
        encoder.set_bytes(
            3,
            8,
            &(dag_len_mask as u64).to_le_bytes() as *const _ as *const std::ffi::c_void,
        );
        ...
        let thread_groups = MTLSize::new(
            batch_size.div_ceil(256) as u64, // Round up division
            1,
            1,
        );
...
lazy_static! {
    pub static ref METAL_GPU_CONTEXT: Mutex<Option<MetalGpuContext>> = {
        match MetalGpuContext::new() {
            ...
            Err(e) => {
                warn!(
                    "[Qanhash-Metal] Failed to initialize Metal GPU context: {}",
                    e
                );
                Mutex::new(None)
            }
