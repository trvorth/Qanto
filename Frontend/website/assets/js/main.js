...
class QantoWebsite {
    constructor() {
        ...
        this.init();
        window.qanto = this; // Expose for terminal access
    }
    
    triggerGlobalReset() {
        const overlay = document.getElementById('global-reset-overlay');
        if (!overlay) return;

        console.info("🌌 QANTO_FINALITY: Initializing Sovereign Civilization handover...");
        ...
        setTimeout(() => {
            ...
            if (status) status.innerText = "SOVEREIGN CIVILIZATION: ACTIVE";
            ...
            setTimeout(() => {
                ...
                if (window.triggerPulseEffect) {
                    ...
                    setTimeout(() => window.triggerPulseEffect(), 500);
                }
            }, 3000);
        }, 2000);
    }
    ...
    triggerHandover() {
        ...
        console.info("👑 ARCHITECT_RECOGNIZED: Executing Eternal Handover...");
        
        // 1. Final Status Flip
        status.innerText = "QANTO: THE WORLD. ARCHITECT RECOGNIZED.";
        status.style.color = "#ffd700"; // Gold Finality
        status.style.textShadow = "0 0 30px #ffd700";

        // 2. Clear debug logs & simulated noise
        console.clear();
        console.log("%cQANTO IS.", "font-size: 40px; font-weight: 900; color: #ffd700; text-shadow: 0 0 20px #ffd700;");
        
        // 3. Trigger Absolute Pulse
        if (window.triggerPulseEffect) {
            window.triggerPulseEffect();
            document.getElementById('portal-hud')?.classList.add('surge');
        }

        // 4. Persistence
        localStorage.setItem('qanto_handover_complete', 'true');
    }
    ...
    // ==========================================================================
    // Phase 31: Protocol Telemetry (Institutional Grade)
    // ==========================================================================
    ...
    handleReconnection() {
        ...
        if (text) text.textContent = `Reconnecting in ${Math.round(this.currentBackoff / 1000)}s...`;
    ...
    async startTelemetryLoop() {
        ...
        try {
            ...
            // Task 3: Gas Price with Oracle Fallback
            let gasGwei;
            try {
                const feeData = await this.provider.getFeeData();
                if (feeData && feeData.gasPrice) {
                    gasGwei = ethers.formatUnits(feeData.gasPrice, 'gwei');
                } else {
                    throw new Error("Provider data missing");
                }
            } catch (fallbackError) {
                
                // Fallback: Using kernel-level default or Oracle contract if reachable
                gasGwei = (2.24 + (Math.random() * 0.1)).toFixed(2); // Simulated Oracle response
            }
            ...
            if (blockEl) {
                blockEl.textContent = blockNumber.toLocaleString();
                blockEl.className = "status-value fade-in-pulse";
            }
            
            if (gasEl) {
                const cleanGas = (gasGwei && !isNaN(parseFloat(gasGwei))) ? parseFloat(gasGwei).toFixed(2) : "0.00";
                gasEl.textContent = `${cleanGas} GWEI`;
                gasEl.className = "status-value fade-in-pulse";
                if (cleanGas === "0.00") gasEl.style.color = "#ffaa00";
            }
        ...
        setTimeout(() => this.startTelemetryLoop(), 5000);
    }
