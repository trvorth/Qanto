...
export const Layout = () => {
  ...
  const navLinks = [
    { name: 'Home', path: 'https://qanto.org', isExternal: true },
    { name: 'Explorer', path: 'https://qanto.org/explorer', isExternal: true },
    { name: 'Doc', path: 'https://qanto.org/doc', isExternal: true },
    { name: 'Ecosystem', path: 'https://qanto.org/ecosystem', isExternal: true },
    { name: 'DEX', path: '/dex', isExternal: false },
    { name: 'Staking', path: '/staking', isExternal: false },
    { name: 'Bridge', path: '/bridge', isExternal: false },
    { name: 'Airdrop', path: '/airdrop', isExternal: false },
    { name: 'Codex', path: '/codex', isExternal: false },
    { name: 'SAGA AI', path: '/saga', isExternal: false }
  ];
    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-cyan-500/30">
      ...
      <nav className="w-full border-b border-white/5 bg-black/50 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          <a href="https://qanto.org" className="text-2xl font-bold tracking-tighter flex items-center gap-2" onClick={() => setIsMobileMenuOpen(false)}>
              ...
              {navLinks.map(link => (
                link.isExternal ? (
                  <a 
                    ...
                    className="w-full text-center py-3 rounded-xl text-base font-semibold text-slate-400 hover:text-white hover:bg-white/5 transition-all"
                  >
                    {link.name}
                  </a>
                ) : (
