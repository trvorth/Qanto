...
fn run_analysis(risk_type: &str) {
    match risk_type {
        "centralization" => {
            ...
            print_with_delay("-- Fetching block production data for last 10 epochs...", 200);
            print_with_delay("! SecurityMonitor: Calculating Herfindahl-Hirschman Index (HHI)...", 350);
            print_with_delay("HHI calculated: 1850. Moderate concentration detected.", 100);
        ...
        "spam" => {
            print_with_delay("+ Initializing security analysis protocol...", 100);
            print_with_delay("-- Analyzing transaction metadata integrity for the last 200 blocks...", 250);
            print_with_delay("-- Calculating zero-fee transaction ratio...", 250);
            print_with_delay("! SecurityMonitor: Zero-fee ratio at 12%. Shannon entropy of metadata is nominal.", 350);
            print_with_delay("✓ Analysis complete. No indicators of a coordinated spam attack detected.", 100);
        }
        _ => {
...
fn run_simulation() {
    print_with_delay("+ Starting Sense-Think-Act loop for epoch governance...", 100);
    print_with_delay("[Sense] Analyzing long-term network trends...", 200);
    print_with_delay("-- Trend detected: Sustained network congestion (avg > 0.85 for 3 epochs).", 250);
    print_with_delay("-- Trend detected: High proposal rejection rate (78%).", 250);
    print_with_delay("[Think] Formulating strategic proposals...", 200);
    print_with_delay("! Proposal Drafted: Increase 'base_tx_fee_min' by 15% to mitigate spam.", 350);
    ...
    print_with_delay("✓ Autonomous governance cycle complete. 2 new proposals submitted.", 100);
}
...
fn process_command(input: &str) {
    ...
    match *command {
        "/help" => {
            ...
            println!("  /analyze [risk]    - Run security analysis (e.g., /analyze centralization)");
        ...
        _ => {
            if command.starts_with('/') {
                 ...
                 println!("Invalid input. All commands must start with a forward slash (e.g., /help).");
            }
        }
    }
}
