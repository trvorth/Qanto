...
pub enum WalletError {
    ...
    AtomicSave(String),
    #[error("Wallet stream not connected")]
    StreamNotConnected,
}
...
pub struct Wallet {
    ...
    #[zeroize(skip)]
    #[allow(unused_assignments)]
    pub stream_tx: Option<broadcast::Sender<BalanceEvent>>, // non-serialized
    #[zeroize(skip)]
    pub current_balance: Arc<RwLock<Balance>>, // non-serialized
}
...
impl Wallet {
    ...
    pub fn new() -> Result<Self, WalletError> {
        ...
        let mut w = Self::from_mnemonic(mnemonic.to_string().as_str())?;
        w.current_balance = Arc::new(RwLock::new(Balance::default()));
        Ok(w)
    }
    ...
    pub fn from_private_key(private_key_hex: &str) -> Result<Self, WalletError> {
        ...
        Ok(Self {
            data: WalletData {
                ...
                qr_secret_key: SecretVec::new(pq_sk.as_bytes().to_vec()),
                qr_public_key: pq_pk.as_bytes().to_vec(),
                mnemonic: Secret::new("imported-no-mnemonic".to_string()),
            },
            stream_tx: None,
            current_balance: Arc::new(RwLock::new(Balance::default())),
        })
    }

    /// Expose the PQ secret key as hex (FOR DEBUGGING/RECOVERY ONLY)
    pub fn expose_secret_key(&self) -> String {
        hex::encode(self.data.qr_secret_key.expose_secret())
    }
    ...
    /// Derives a child wallet at a specific index using a custom deterministic scheme.
    /// Scheme: Child_Seed = HMAC-SHA256(Key = Root_Seed, Data = "QANTO_HD" || Index)
    /// This allows generating multiple addresses from the same mnemonic.
    pub fn derive_child(&self, index: u32) -> Result<Self, WalletError> {
        use hmac::{Hmac, Mac};
        use sha2::Sha256;

        let mnemonic_str = self.data.mnemonic.expose_secret();
        if mnemonic_str.is_empty() {
            return Err(WalletError::Crypto(
                "Cannot derive from wallet without mnemonic".to_string(),
            ));
        }

        let mnemonic = Mnemonic::parse(mnemonic_str)?;
        let root_seed = mnemonic.to_seed(""); // 64 bytes

        type HmacSha256 = Hmac<Sha256>;
        let mut mac = <HmacSha256 as KeyInit>::new_from_slice(&root_seed)
            .map_err(|_| WalletError::Crypto("HMAC initialization failed".to_string()))?;

        mac.update(b"QANTO_HD");
        mac.update(&index.to_be_bytes());

        let result = mac.finalize();
        let derived_bytes = result.into_bytes(); // 32 bytes
        let derived_array: [u8; 32] = derived_bytes.into();

        // Use the derived bytes as the seed for Ed25519
        // For PQ, we hash it again to get a different seed
        let mut mac_pq = <HmacSha256 as KeyInit>::new_from_slice(&root_seed)
            .map_err(|_| WalletError::Crypto("HMAC initialization failed".to_string()))?;
        mac_pq.update(b"QANTO_HD_PQ");
        mac_pq.update(&index.to_be_bytes());
        let result_pq = mac_pq.finalize();
        let derived_bytes_pq = result_pq.into_bytes();
        let derived_array_pq: [u8; 32] = derived_bytes_pq.into();

        let signing_key = Ed25519SigningKey::from_bytes(&derived_array)
            .map_err(|e| WalletError::Crypto(e.to_string()))?;
        let verifying_key = signing_key.verifying_key();

        let (pk, sk) = generate_pq_keypair(Some(derived_array_pq))
            .map_err(|e| WalletError::PqCrypto(e.to_string()))?;
        ...
        Ok(Self {
            data: WalletData {
                ...
                qr_secret_key: SecretVec::new(sk.as_bytes().to_vec()),
                qr_public_key: pk.as_bytes().to_vec(),
                mnemonic: Secret::new(mnemonic_str.clone()), // Keep original mnemonic
            },
            stream_tx: None,
            current_balance: Arc::new(RwLock::new(Balance::default())),
        })
    ...
    /// Connects to the node's WebSocket API and streams balance updates.
    pub async fn connect_stream(&mut self, api_addr: &str) -> Result<(), WalletError> {
                ...
                if let Ok(tokio_tungstenite::tungstenite::Message::Text(text)) = msg {
                    if let Ok(v) = serde_json::from_str::<serde_json::Value>(&text) {
                        ...
                        let event = match msg_type {
                            Some("balance_snapshot") => Some(BalanceEvent::Snapshot {
                                confirmed: v["total_confirmed"].as_u64().unwrap_or(0),
                                unconfirmed: v["total_unconfirmed"].as_u64().unwrap_or(0),
                                ts: v["timestamp"].as_u64().unwrap_or(0),
                            }),
                            Some("balance_update") => Some(BalanceEvent::Update {
                                spendable_confirmed: v["data"]["spendable_confirmed"]
                                    .as_u64()
                                    .unwrap_or(0),
                                immature_coinbase_confirmed: v["data"]
                                    ["immature_coinbase_confirmed"]
                                    .as_u64()
                                    .unwrap_or(0),
                                unconfirmed_delta: v["data"]["unconfirmed_delta"]
                                    .as_i64()
                                    .unwrap_or(0),
                                total_confirmed: v["data"]["total_confirmed"].as_u64().unwrap_or(0),
                                ts: v["timestamp"].as_u64().unwrap_or(0),
                            }),
                            _ => None,
                        };
    ...
    pub async fn start_real_time_observer(&mut self, api_addr: &str) -> Result<(), WalletError> {
        self.connect_stream(api_addr).await?;
        let rx = self
            .stream_tx
            .as_ref()
            .ok_or(WalletError::StreamNotConnected)?
            .subscribe();
        let balance_lock = self.current_balance.clone();
        tokio::spawn(async move {
            let mut rx = rx;
            loop {
                match rx.recv().await {
                    Ok(BalanceEvent::Snapshot {
                        confirmed,
                        unconfirmed,
                        ts,
                    }) => {
                        let mut b = balance_lock.write().unwrap();
                        b.total_confirmed = confirmed;
                        b.unconfirmed_delta = unconfirmed as i64;
                        b.ts = ts;
                        let base = crate::transaction::SMALLEST_UNITS_PER_QAN as f64;
                        let v = b.total_confirmed as f64 / base;
                        println!("\r📊 Real-Time Balance: {:.6} QANTO", v);
                    }
                    Ok(BalanceEvent::Update {
                        spendable_confirmed,
                        immature_coinbase_confirmed,
                        unconfirmed_delta,
                        total_confirmed,
                        ...
                        let mut b = balance_lock.write().unwrap();
                        b.spendable_confirmed = spendable_confirmed;
                        b.immature_coinbase_confirmed = immature_coinbase_confirmed;
                        b.unconfirmed_delta = unconfirmed_delta;
                        b.total_confirmed = total_confirmed;
                        b.ts = ts;
                        let base = crate::transaction::SMALLEST_UNITS_PER_QAN as f64;
                        let v = b.total_confirmed as f64 / base;
                        println!("\r📊 Real-Time Balance: {:.6} QANTO", v);
                    }
                    Ok(BalanceEvent::Delta { .. }) => {}
                    Err(_) => break,
                }
            }
        });
