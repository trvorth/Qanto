...
use tracing::{span, Dispatch, Subscriber};
use tracing_core::{Metadata, span as core_span};
...
pub(crate) struct WithContext(
    fn(&Dispatch, &core_span::Id, f: &mut dyn FnMut(&'static Metadata<'static>, &str) -> bool),
);
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    /// `Attributes` and `Id`.
    fn on_new_span(&self, _attrs: &core_span::Attributes<'_>, _id: &core_span::Id, _ctx: layer::Context<'_, S>) {
    }
