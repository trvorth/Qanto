use super::*;
use crate::crypto::qanhash::{Target, MIX_BYTES, get_qdag};
use crate::qanto_native_crypto::QantoHash;
use lazy_static::lazy_static;
use log::{debug, info, warn};
use metal::{
    Buffer, CommandQueue, ComputeCommandEncoder, ComputePipelineState, Device, MTLResourceOptions,
    MTLSize,
};
use std::sync::Mutex;
...
impl MetalGpuContext {
    ...
    pub fn hash_batch(
        ...
        let thread_groups = MTLSize::new(
            ((batch_size + 255) / 256) as u64, // Round up division
            1,
            1,
        );
...
lazy_static! {
    pub static ref METAL_GPU_CONTEXT: Mutex<Option<MetalGpuContext>> = {
        match MetalGpuContext::new() {
            ...
            Err(e) => {
                debug!("[Qanhash-Metal] Failed to initialize Metal GPU context: {}", e);
                Mutex::new(None)
            }
        }
    };
