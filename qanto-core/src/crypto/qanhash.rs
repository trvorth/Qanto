...
use crate::crypto::error::QanhashError;
...
#[inline]
pub fn mine_simd_batch(
    header_hash: &QantoHash,
    start_nonce: u64,
    lanes: usize,
) -> Option<(u64, [u8; 32])> {
    // Optimized SIMD-friendly mining
    // We process 4 nonces at a time (AVX2/NEON width friendly)
    // Since input is 40 bytes (32 header + 8 nonce), it fits in one 64-byte block with padding.
    // Block layout: [Header(32) | Nonce(8) | 0x80 | Zeros... | Len(8)]

    let mut template = [0u8; 64];
    template[0..32].copy_from_slice(header_hash.as_bytes());
    template[40] = 0x80;
    // Total len = 320 bits (40 bytes * 8)
    let bit_len = 320u64.to_be_bytes();
    template[56..64].copy_from_slice(&bit_len);

    let mut nonce = start_nonce;
    let end_nonce = start_nonce + lanes as u64;

    // Process in chunks of 4
    while nonce < end_nonce {
        let remaining = end_nonce - nonce;
        if remaining < 4 {
            // Scalar fallback for leftovers
            for i in 0..remaining {
                let curr_nonce = nonce + i;
                template[32..40].copy_from_slice(&curr_nonce.to_le_bytes());

                let mut hasher = QantoHasher::new();
                hasher.process_block(&template);

                // Extract result (first 32 bytes of state)
                let mut result = [0u8; 32];
                for (j, &state_word) in hasher.state[..4].iter().enumerate() {
                    result[j * 8..(j + 1) * 8].copy_from_slice(&state_word.to_be_bytes());
                }

                // Check difficulty (assuming target is checked externally or we just check for non-zero which implies work done?
                // Wait, original code checked `arr[0] == 0`. We stick to that heuristic or need the target.
                // The original code: `if arr[0] == 0 { return Some((nonce, arr)); }`
                if result[0] == 0 {
                    return Some((curr_nonce, result));
                }
            }
            break;
        }

        // SIMD-like processing for 4 lanes
        let n0 = nonce;
        let n1 = nonce + 1;
        let n2 = nonce + 2;
        let n3 = nonce + 3;

        // Prepare 4 states (manual unroll for autovectorization)
        // Initial state from QantoHasher::new()
        let init_state: [u64; 8] = [
            0x6a09e667f3bcc908,
            0xbb67ae8584caa73b,
            0x3c6ef372fe94f82b,
            0xa54ff53a5f1d36f1,
        ...
        let mut s0 = init_state;
        let mut s1 = init_state;
        let mut s2 = init_state;
        let mut s3 = init_state;

        // Prepare blocks
        let mut b0 = template;
        b0[32..40].copy_from_slice(&n0.to_le_bytes());
        let mut b1 = template;
        b1[32..40].copy_from_slice(&n1.to_le_bytes());
        let mut b2 = template;
        b2[32..40].copy_from_slice(&n2.to_le_bytes());
        let mut b3 = template;
        b3[32..40].copy_from_slice(&n3.to_le_bytes());

        // Process blocks (inlined QantoHasher::process_block logic)
        // We do this to allow the compiler to interleave instructions

        // Helper macro to process one round for 4 states
        macro_rules! process_lane {
            ($state:expr, $block:expr) => {
                ...
                for round in 0..4 {
                    for i in 0..8 {
                        let next = (i + 1) % 8;
                        let prev = (i + 7) % 8;
                        let s_curr: u64 = $state[i];
                        let s_next: u64 = $state[next];
                        let s_prev: u64 = $state[prev];
        ...
        // Check results
        let check_res = |state: [u64; 8]| -> Option<[u8; 32]> {
            let mut res = [0u8; 32];
            for (i, &word) in state[..4].iter().enumerate() {
                res[i * 8..(i + 1) * 8].copy_from_slice(&word.to_be_bytes());
            }
            if res[0] == 0 {
                Some(res)
            } else {
        ...
        if let Some(r) = check_res(s0) {
            return Some((n0, r));
        }
        if let Some(r) = check_res(s1) {
            return Some((n1, r));
        }
        if let Some(r) = check_res(s2) {
            return Some((n2, r));
        }
        if let Some(r) = check_res(s3) {
            return Some((n3, r));
        }
...
const CPU_BATCH_SIZE: usize = 4096;
...
#[cfg(feature = "gpu")]
pub use gpu_impl::{hash_batch, hash_batch_with_dag, GpuContext, GPU_CONTEXT};

#[cfg(feature = "gpu")]
pub fn gpu_warmup() -> Result<(), Box<dyn std::error::Error>> {
    gpu_impl::ensure_gpu_context()?;
    let mut header = [0u8; 32];
    header[0..8].copy_from_slice(&0u64.to_le_bytes());
    let header_hash = QantoHash::new(header);
    let _ = gpu_impl::hash_batch(&header_hash, 0, 256, [0u8; 32])?;
    Ok(())
}
...
mod gpu_impl {
    ...
    use opencl3::{
        ...
        memory::{
            Buffer, CL_MEM_COPY_HOST_PTR, CL_MEM_READ_ONLY, CL_MEM_READ_WRITE, CL_MEM_USE_HOST_PTR,
        },
        program::Program,
    ...
    const DEFAULT_LOCAL_WORK_SIZE: usize = 256;
    ...
    impl GpuContext {
        fn get_max_work_group_size(device: &Device) -> Result<usize, Box<dyn std::error::Error>> {
            Ok(device.max_work_group_size()?)
        }
        ...
        fn new(kernel_src: &str) -> Result<Self, Box<dyn std::error::Error>> {
            ...
            let max_work_group_size = Self::get_max_work_group_size(&device)?;
            ...
            info!(
                "[Qanhash-GPU] Initialized OpenCL context on device: {} (Max WorkGroup: {})",
                device.name()?,
                max_work_group_size
            );
            Ok(Self {
    ...
    lazy_static! {
        pub static ref GPU_CONTEXT: Mutex<Option<GpuContext>> = {
            info!("[Qanhash-GPU] Compiling OpenCL kernel...");
            let ctx = GpuContext::new(KERNEL_SRC).ok();
            Mutex::new(ctx)
        };
    }

    pub fn ensure_gpu_context() -> Result<(), Box<dyn std::error::Error>> {
        let mut guard = GPU_CONTEXT.lock().unwrap_or_else(|e| e.into_inner());
        if guard.is_some() {
            return Ok(());
        }
        let ctx = GpuContext::new(KERNEL_SRC)?;
        *guard = Some(ctx);
        Ok(())
    }
    ...
    pub fn hash_batch(
        ...
        unsafe {
            let gpu_guard = GPU_CONTEXT.lock().unwrap_or_else(|e| e.into_inner());
            let Some(gpu) = gpu_guard.as_ref() else {
                return Err("OpenCL GPU not available".into());
            };
            ...
            let max_work_group_size = gpu.max_work_group_size.max(1);
            let local_work_size = DEFAULT_LOCAL_WORK_SIZE.min(max_work_group_size);
            if max_work_group_size < DEFAULT_LOCAL_WORK_SIZE {
                warn!(
                    "[Qanhash-GPU] Clamping local work size to device limit: {}",
                    max_work_group_size
                );
            }
            ...
            let global_work_size =
                (batch_size + local_work_size - 1).div_ceil(local_work_size) * local_work_size;
...
#[inline]
fn meets_target_limb_be(hash: &[u8; 32], target: &[u8; 32]) -> bool {
    let mut i = 0;
    while i < 32 {
...
pub fn is_solution_valid(hash: &[u8; 32], target: Target) -> bool {
    meets_target_limb_be(hash, &target)
}
...
impl CpuMiner {
    ...
    pub fn mine(
        ...
        println!(
            "DEBUG: CpuMiner::mine_with_dag started with {} batches",
            total_batches
        );
    ...
    /// SIMD-optimized batch mining for x86_64 with AVX2/AVX-512
    #[cfg(target_arch = "x86_64")]
    fn mine_batch_simd(
        header_hash: &QantoHash,
        start_nonce: u64,
    ...
    unsafe fn mine_batch_avx2(
        ...
        // Helper: rotate left for 64-bit lanes
        #[inline(always)]
        unsafe fn rotl64_var(v: __m256i, r: i32) -> __m256i {
            match r {
                1 => _mm256_or_si256(_mm256_slli_epi64::<1>(v), _mm256_srli_epi64::<63>(v)),
                2 => _mm256_or_si256(_mm256_slli_epi64::<2>(v), _mm256_srli_epi64::<62>(v)),
                3 => _mm256_or_si256(_mm256_slli_epi64::<3>(v), _mm256_srli_epi64::<61>(v)),
                4 => _mm256_or_si256(_mm256_slli_epi64::<4>(v), _mm256_srli_epi64::<60>(v)),
                5 => _mm256_or_si256(_mm256_slli_epi64::<5>(v), _mm256_srli_epi64::<59>(v)),
                6 => _mm256_or_si256(_mm256_slli_epi64::<6>(v), _mm256_srli_epi64::<58>(v)),
                7 => _mm256_or_si256(_mm256_slli_epi64::<7>(v), _mm256_srli_epi64::<57>(v)),
                8 => _mm256_or_si256(_mm256_slli_epi64::<8>(v), _mm256_srli_epi64::<56>(v)),
                _ => v,
            }
        }

        // Process 4 nonces simultaneously using AVX2 64-bit lanes
        const LANES: usize = 4;
        for chunk_start in (0..batch_size).step_by(LANES) {
            if should_stop.load(Ordering::Relaxed) {
                return None;
            }
            let lane_count = (chunk_start + LANES).min(batch_size) - chunk_start;

            // Initialize mix vectors for 16 lanes of 64-bit state split across 4 registers
            // mix[0..3]
            let header_bytes = header_hash.as_bytes();
            let mut mix0 = _mm256_set1_epi64x(u64::from_le_bytes([
                header_bytes[0],
                header_bytes[1],
                header_bytes[2],
                header_bytes[3],
            ...
            let mut mix1 = _mm256_set1_epi64x(u64::from_le_bytes([
                header_bytes[8],
                header_bytes[9],
                header_bytes[10],
                header_bytes[11],
            ...
            let mut mix2 = _mm256_set1_epi64x(u64::from_le_bytes([
                header_bytes[16],
                header_bytes[17],
                header_bytes[18],
                header_bytes[19],
            ...
            let mut mix3 = _mm256_set1_epi64x(u64::from_le_bytes([
                header_bytes[24],
                header_bytes[25],
                header_bytes[26],
                header_bytes[27],
            ...
            // mix[4] holds nonce per lane
            let mut mix4 = {
                let n0 = (start_nonce + chunk_start as u64) as i64;
                let n1 = (start_nonce + (chunk_start + 1).min(batch_size - 1) as u64) as i64;
                let n2 = (start_nonce + (chunk_start + 2).min(batch_size - 1) as u64) as i64;
                let n3 = (start_nonce + (chunk_start + 3).min(batch_size - 1) as u64) as i64;
                _mm256_set_epi64x(n3, n2, n1, n0)
            };

            // Remaining mix lanes kept in scalar arrays but updated with AVX2 where possible
            let mut mix_rest = [0u64; 12];
            // Initialize remaining mix lanes to zero for consistency with scalar impl
            for m in mix_rest.iter_mut() {
                *m = 0;
            }

            // 32 rounds
            for _round in 0..32 {
                // p_index per lane = mix0 + mix1
                let p_index_vec = _mm256_add_epi64(mix0, mix1);
                // Extract lane indices
                let mut p_idx = [0u64; LANES];
                p_idx[0] = _mm256_extract_epi64::<0>(p_index_vec) as u64;
                p_idx[1] = _mm256_extract_epi64::<1>(p_index_vec) as u64;
                p_idx[2] = _mm256_extract_epi64::<2>(p_index_vec) as u64;
                p_idx[3] = _mm256_extract_epi64::<3>(p_index_vec) as u64;

                // Load dag entries for k = 0..7 into vectors for two groups
                for k in 0..8 {
                    let d1_0 = unsafe {
                        *(dag[p_idx[0] as usize & dag_len_mask].as_ptr() as *const u64).add(k)
                    };
                    let d1_1 = unsafe {
                        *(dag[p_idx[1] as usize & dag_len_mask].as_ptr() as *const u64).add(k)
                    };
                    let d1_2 = unsafe {
                        *(dag[p_idx[2] as usize & dag_len_mask].as_ptr() as *const u64).add(k)
                    };
                    let d1_3 = unsafe {
                        *(dag[p_idx[3] as usize & dag_len_mask].as_ptr() as *const u64).add(k)
                    };
                    let d2_0 = unsafe {
                        *(dag[(p_idx[0].wrapping_add(1)) as usize & dag_len_mask].as_ptr()
                            as *const u64)
                            .add(k)
                    };
                    let d2_1 = unsafe {
                        *(dag[(p_idx[1].wrapping_add(1)) as usize & dag_len_mask].as_ptr()
                            as *const u64)
                            .add(k)
                    };
                    let d2_2 = unsafe {
                        *(dag[(p_idx[2].wrapping_add(1)) as usize & dag_len_mask].as_ptr()
                            as *const u64)
                            .add(k)
                    };
                    let d2_3 = unsafe {
                        *(dag[(p_idx[3].wrapping_add(1)) as usize & dag_len_mask].as_ptr()
                            as *const u64)
                            .add(k)
                    };

                    let d1v = _mm256_set_epi64x(d1_3 as i64, d1_2 as i64, d1_1 as i64, d1_0 as i64);
                    let d2v = _mm256_set_epi64x(d2_3 as i64, d2_2 as i64, d2_1 as i64, d2_0 as i64);

                    // Update the first 5 lanes using vector ops mirroring scalar logic
                    // x = x*31 + d1; rot; xor d2
                    // mix0
                    let x0_shl5 = _mm256_slli_epi64::<5>(mix0);
                    let x0_mul31 = _mm256_sub_epi64(x0_shl5, mix0);
                    let x0_add = _mm256_add_epi64(x0_mul31, d1v);
                    let x0_rot = rotl64_var(x0_add, if k < 4 { 1 + k as i32 } else { 1 });
                    mix0 = _mm256_xor_si256(x0_rot, d2v);

                    // mix1
                    let x1_shl5 = _mm256_slli_epi64::<5>(mix1);
                    let x1_mul31 = _mm256_sub_epi64(x1_shl5, mix1);
                    let x1_add = _mm256_add_epi64(x1_mul31, d1v);
                    let x1_rot = rotl64_var(x1_add, if k < 4 { 2 + k as i32 } else { 2 });
                    mix1 = _mm256_xor_si256(x1_rot, d2v);

                    // mix2
                    let x2_shl5 = _mm256_slli_epi64::<5>(mix2);
                    let x2_mul31 = _mm256_sub_epi64(x2_shl5, mix2);
                    let x2_add = _mm256_add_epi64(x2_mul31, d1v);
                    let x2_rot = rotl64_var(x2_add, if k < 4 { 3 + k as i32 } else { 3 });
                    mix2 = _mm256_xor_si256(x2_rot, d2v);

                    // mix3
                    let x3_shl5 = _mm256_slli_epi64::<5>(mix3);
                    let x3_mul31 = _mm256_sub_epi64(x3_shl5, mix3);
                    let x3_add = _mm256_add_epi64(x3_mul31, d1v);
                    let x3_rot = rotl64_var(x3_add, if k < 4 { 4 + k as i32 } else { 4 });
                    mix3 = _mm256_xor_si256(x3_rot, d2v);

                    // mix4 (nonce state)
                    let x4_shl5 = _mm256_slli_epi64::<5>(mix4);
                    let x4_mul31 = _mm256_sub_epi64(x4_shl5, mix4);
                    let x4_add = _mm256_add_epi64(x4_mul31, d1v);
                    let x4_rot = rotl64_var(x4_add, if k < 4 { 5 + k as i32 } else { 5 });
                    mix4 = _mm256_xor_si256(x4_rot, d2v);
                }

                // Update remaining scalar lanes 5..15 to match scalar semantics
                for k in 8..16 {
                    let p_index0 = p_idx[0];
                    let dag_entry1: &[u64; 16] = unsafe {
                        &*(dag[p_index0 as usize & dag_len_mask].as_ptr() as *const [u64; 16])
                    };
                    let dag_entry2: &[u64; 16] = unsafe {
                        &*(dag[(p_index0.wrapping_add(1)) as usize & dag_len_mask].as_ptr()
                            as *const [u64; 16])
                    };
                    let idx = k as usize;
                    let r = if idx <= 7 {
                    ...
                    let m = &mut mix_rest[idx - 4];
                    *m = m
                        .wrapping_mul(31)
                        .wrapping_add(dag_entry1[idx])
                        .rotate_left(r)
                        ^ dag_entry2[idx];
                }
            }

            // After rounds, evaluate each lane's hash and target
            let mut lanes_nonce = [0u64; LANES];
            lanes_nonce[0] = _mm256_extract_epi64::<0>(mix4) as u64;
            lanes_nonce[1] = _mm256_extract_epi64::<1>(mix4) as u64;
            lanes_nonce[2] = _mm256_extract_epi64::<2>(mix4) as u64;
            lanes_nonce[3] = _mm256_extract_epi64::<3>(mix4) as u64;

            for lane in 0..lane_count {
                let nonce = lanes_nonce[lane];
                let result_hash =
    ...
    fn mine_batch_scalar(
        ...
        for i in 0..batch_size {
            ...
            if is_solution_valid(&result_hash, target) {
                return Some((nonce, result_hash));
            } else if target[0] == 0xff {
                println!(
                    "DEBUG: IMPOSSIBLE SCALAR! Hash {:?} > Target {:?}",
    ...
    fn mine_batch_scalar_with_dag(
        ...
        println!(
            "DEBUG: Entering mine_batch_scalar_with_dag nonce={}",
            start_nonce
        );
        let dag_len_mask = dag.len() - 1;
    ...
    unsafe fn mine_batch_avx2_with_dag(
        ...
        // println!("DEBUG: Entering mine_batch_neon_with_dag nonce={}", start_nonce);
        if dag.is_empty() {
            println!("DEBUG: DAG IS EMPTY!");
            return None;
        }
        let dag_len_mask = dag.len() - 1;
        ...
        for chunk_start in (0..batch_size).step_by(LANES) {
            ...
            for _round in 0..32 {
                ...
                for k in 0..8 {
                    ...
                    let x0 = _mm256_sub_epi64(_mm256_slli_epi64::<5>(mix0), mix0);
                    let x1 = _mm256_sub_epi64(_mm256_slli_epi64::<5>(mix1), mix1);
                    let x2 = _mm256_sub_epi64(_mm256_slli_epi64::<5>(mix2), mix2);
                    let x3 = _mm256_sub_epi64(_mm256_slli_epi64::<5>(mix3), mix3);
                    let x4 = _mm256_sub_epi64(_mm256_slli_epi64::<5>(mix4), mix4);

                    mix0 = _mm256_xor_si256(
                        rotl64_var(
                            _mm256_add_epi64(x0, d1v),
                            if k < 4 { 1 + k as i32 } else { 1 },
                        ),
                        d2v,
                    );
                    mix1 = _mm256_xor_si256(
                        rotl64_var(
                            _mm256_add_epi64(x1, d1v),
                            if k < 4 { 2 + k as i32 } else { 2 },
                        ),
                        d2v,
                    );
                    mix2 = _mm256_xor_si256(
                        rotl64_var(
                            _mm256_add_epi64(x2, d1v),
                            if k < 4 { 3 + k as i32 } else { 3 },
                        ),
                        d2v,
                    );
                    mix3 = _mm256_xor_si256(
                        rotl64_var(
                            _mm256_add_epi64(x3, d1v),
                            if k < 4 { 4 + k as i32 } else { 4 },
                        ),
                        d2v,
                    );
                    mix4 = _mm256_xor_si256(
                        rotl64_var(
                            _mm256_add_epi64(x4, d1v),
                            if k < 4 { 5 + k as i32 } else { 5 },
                        ),
                        d2v,
                    );
                }
            ...
            // Extract per-lane nonces and verify
            let mut lanes_nonce = [0u64; LANES];
            lanes_nonce[0] = _mm256_extract_epi64::<0>(mix4) as u64;
            lanes_nonce[1] = _mm256_extract_epi64::<1>(mix4) as u64;
            lanes_nonce[2] = _mm256_extract_epi64::<2>(mix4) as u64;
    ...
    fn mine_batch_simd_with_dag(
        ...
        #[cfg(target_arch = "aarch64")]
        {
            if is_aarch64_feature_detected!("neon") {
                unsafe {
                    return Self::mine_batch_neon_with_dag(
                        header_hash,
                        start_nonce,
                        batch_size,
                        target,
    ...
    /// NEON-optimized batch mining for aarch64
    #[cfg(target_arch = "aarch64")]
    unsafe fn mine_batch_neon(
        header_hash: &QantoHash,
        start_nonce: u64,
        batch_size: usize,
        target: Target,
        ...
        #[inline(always)]
        unsafe fn rotl64(v: uint64x2_t, r: i32) -> uint64x2_t {
            // Variable rotate via vshlq_u64 with per-lane shift counts
            let shl = vdupq_n_s64(r as i64);
            let shr = vdupq_n_s64(-(64 - r) as i64);
            let l = vshlq_u64(v, shl);
            let rr = vshlq_u64(v, shr);
            vorrq_u64(l, rr)
        }
        ...
        for chunk_start in (0..batch_size).step_by(LANES) {
            ...
            let header_bytes = header_hash.as_bytes();
            let mix0 = vdupq_n_u64(u64::from_le_bytes([
                header_bytes[0],
                header_bytes[1],
                header_bytes[2],
                header_bytes[3],
            ...
            let mix1 = vdupq_n_u64(u64::from_le_bytes([
                header_bytes[8],
                header_bytes[9],
                header_bytes[10],
                header_bytes[11],
            ...
            let mix2 = vdupq_n_u64(u64::from_le_bytes([
                header_bytes[16],
                header_bytes[17],
                header_bytes[18],
                header_bytes[19],
            ...
            let mix3 = vdupq_n_u64(u64::from_le_bytes([
                header_bytes[24],
                header_bytes[25],
                header_bytes[26],
                header_bytes[27],
            ...
            let mut mix4 = vsetq_lane_u64(start_nonce + chunk_start as u64, vdupq_n_u64(0), 0);
            mix4 = vsetq_lane_u64(
                start_nonce + ((chunk_start + 1).min(batch_size - 1) as u64),
                mix4,
                1,
            );

            for _round in 0..32 {
                let p_index = vaddq_u64(mix0, mix1);
                let p0 = vgetq_lane_u64(p_index, 0);
                let p1 = vgetq_lane_u64(p_index, 1);
                for k in 0..8 {
                    let d1_0 =
                        unsafe { *(dag[p0 as usize & dag_len_mask].as_ptr() as *const u64).add(k) };
                    let d1_1 =
                        unsafe { *(dag[p1 as usize & dag_len_mask].as_ptr() as *const u64).add(k) };
                    let d2_0 = unsafe {
                        *(dag[(p0.wrapping_add(1)) as usize & dag_len_mask].as_ptr() as *const u64)
                            .add(k)
                    };
                    let d2_1 = unsafe {
                        *(dag[(p1.wrapping_add(1)) as usize & dag_len_mask].as_ptr() as *const u64)
                            .add(k)
                    };
                    let d1v = vsetq_lane_u64(d1_1, vdupq_n_u64(d1_0), 1);
                    let d2v = vsetq_lane_u64(d2_1, vdupq_n_u64(d2_0), 1);

                    // mul by 31 via (x<<5) - x
                    let x0 = vsubq_u64(vshlq_n_u64(mix0, 5), mix0);
                    let x1 = vsubq_u64(vshlq_n_u64(mix1, 5), mix1);
                    let x2 = vsubq_u64(vshlq_n_u64(mix2, 5), mix2);
                    let x3 = vsubq_u64(vshlq_n_u64(mix3, 5), mix3);
                    let x4 = vsubq_u64(vshlq_n_u64(mix4, 5), mix4);

                    let r0 = rotl64(vaddq_u64(x0, d1v), if k < 4 { 1 + k as i32 } else { 1 });
                    let r1 = rotl64(vaddq_u64(x1, d1v), if k < 4 { 2 + k as i32 } else { 2 });
                    let r2 = rotl64(vaddq_u64(x2, d1v), if k < 4 { 3 + k as i32 } else { 3 });
                    let r3 = rotl64(vaddq_u64(x3, d1v), if k < 4 { 4 + k as i32 } else { 4 });
                    let r4 = rotl64(vaddq_u64(x4, d1v), if k < 4 { 5 + k as i32 } else { 5 });

                    // XOR with d2
                    let _ = veorq_u64(r0, d2v);
                    let _ = veorq_u64(r1, d2v);
                    let _ = veorq_u64(r2, d2v);
                    let _ = veorq_u64(r3, d2v);
                    mix4 = veorq_u64(r4, d2v);
                }
            }
            ...
            // Lane 0
            if lane_count >= 1 {
                let nonce0 = vgetq_lane_u64::<0>(mix4);
                let result_hash0 =
                    Self::hash_single_optimized(header_hash, nonce0, &dag, dag_len_mask);
                if is_solution_valid(&result_hash0, target) {
                    return Some((nonce0, result_hash0));
                }
            }
            // Lane 1
            if lane_count >= 2 {
                let nonce1 = vgetq_lane_u64::<1>(mix4);
                let result_hash1 =
                    Self::hash_single_optimized(header_hash, nonce1, &dag, dag_len_mask);
                if is_solution_valid(&result_hash1, target) {
                    return Some((nonce1, result_hash1));
                }
            }
        }
        None
    ...
    /// NEON-optimized batch mining with pre-computed DAG for aarch64
    #[cfg(target_arch = "aarch64")]
    unsafe fn mine_batch_neon_with_dag(
        header_hash: &QantoHash,
        start_nonce: u64,
        batch_size: usize,
        target: Target,
        ...
        // println!("DEBUG: NEON target: {:?}", hex::encode(target));
        if start_nonce.is_multiple_of(100_000) {
            println!(
                "DEBUG: NEON target: {:?}, target[0]: {:x}",
                hex::encode(target),
                target[0]
            );
        }
        ...
        for chunk_start in (0..batch_size).step_by(LANES) {
            ...
            if lane_count >= 1 {
                ...
                if is_solution_valid(&result_hash0, target) {
                    ...
                    println!(
                        "DEBUG: IMPOSSIBLE! Hash {:?} > Target {:?}",
                        hex::encode(result_hash0),
                        hex::encode(target)
                    );
                }
            }
...
pub struct QanhashMiner {
    ...
    #[cfg(feature = "gpu")]
    gpu_failure_streak: Arc<AtomicU64>,
    // Add DAG readiness state
    pub dag_readiness: Arc<(std::sync::Mutex<DagReadiness>, Condvar)>,
}
...
impl QanhashMiner {
    ...
    pub fn new(config: MiningConfig) -> Self {
        ...
        let gpu_available = match config.device {
            MiningDevice::Cpu => false,
            _ => {
                // Prefer Metal on macOS
                #[cfg(target_os = "macos")]
                {
                    if metal_gpu::is_metal_available() {
                        info!("[Qanhash] Metal GPU available");
                        true
                    } else {
                        match GPU_CONTEXT.lock() {
                            Ok(guard) if guard.is_some() => {
                                info!("[Qanhash] OpenCL GPU available");
                                true
                            }
                            Err(_) => {
                                log::debug!(
                                    "[Qanhash] No GPU backend available, falling back to CPU"
                                );
                                false
                            }
                            _ => false,
                ...
                #[cfg(not(target_os = "macos"))]
                {
                    match GPU_CONTEXT.lock() {
                        ...
                        Err(_) => {
                            log::debug!("[Qanhash] No GPU backend available, falling back to CPU");
                            false
                        }
                        _ => false,
                    }
        ...
        Self {
            ...
            #[cfg(feature = "gpu")]
            gpu_failure_streak: Arc::new(AtomicU64::new(0)),
            dag_readiness: Arc::new((std::sync::Mutex::new(DagReadiness::Ready), Condvar::new())),
        }
    }

    #[cfg(feature = "gpu")]
    fn record_gpu_success(&self) {
        self.gpu_failure_streak.store(0, Ordering::Relaxed);
    }

    #[cfg(feature = "gpu")]
    fn record_gpu_failure(&self) {
        let streak = self.gpu_failure_streak.fetch_add(1, Ordering::Relaxed) + 1;
        if streak >= 3 && !self.gpu_disabled.swap(true, Ordering::Relaxed) {
            warn!("[Qanhash] GPU circuit breaker tripped, disabling GPU for this session");
        }
    }

    #[cfg(feature = "gpu")]
    fn gpu_enabled_for_session(&self) -> bool {
    ...
    pub fn mine(
        ...
        match device {
            ...
            #[cfg(feature = "gpu")]
            MiningDevice::Gpu => {
                if !self.gpu_enabled_for_session() {
                    if let Some(ref cpu_miner) = self.cpu_miner {
                        return cpu_miner.mine(
                            header_hash,
                            start_nonce,
                            target,
                ...
                if metal_gpu::is_metal_available() {
                    match metal_gpu::metal_hash_batch(header_hash, start_nonce, batch_size, target)
                    {
                        ...
                        Err(e) => {
                            warn!("[Qanhash] Metal GPU mining failed: {e}");
                        }
                    }
                }
                ...
                if cuda_gpu::is_cuda_available() {
                    match cuda_gpu::cuda_hash_batch(header_hash, start_nonce, batch_size, target) {
                        ...
                        Err(e) => {
                            warn!("[Qanhash] CUDA GPU mining failed: {e}");
                        }
                    }
                }
    ...
    fn select_mining_device(&self) -> MiningDevice {
        match self.config.device {
            MiningDevice::Auto => {
                #[cfg(feature = "gpu")]
                {
                    if self.gpu_available && self.gpu_enabled_for_session() {
    ...
    pub fn mine_with_dag(
        ...
        #[cfg(feature = "gpu")]
        let mut device = self.select_mining_device();
        #[cfg(not(feature = "gpu"))]
        let device = self.select_mining_device();

        // Check DAG readiness state
        let (lock, _cvar) = &*self.dag_readiness;
        let readiness = lock.lock().unwrap();

        if *readiness != DagReadiness::Ready {
            #[cfg(feature = "gpu")]
            if device == MiningDevice::Gpu || (device == MiningDevice::Auto && self.gpu_available) {
                warn!(
                    "[Qanhash] DAG not ready on GPU (status: {:?}), forcing CPU fallback",
                    *readiness
                );
                device = MiningDevice::Cpu;
            }
        }
        drop(readiness); // Release lock
        ...
        match device {
            ...
            MiningDevice::Gpu => {
                if !self.gpu_enabled_for_session() {
                    if let Some(ref cpu_miner) = self.cpu_miner {
                        return cpu_miner.mine_with_dag(
                            header_hash,
                            start_nonce,
                            target,
                            self.config.max_iterations,
                ...
                let batch_size = self.config.batch_size.unwrap_or(1024 * 1024);

                // 1. Try Metal on macOS
                #[cfg(target_os = "macos")]
                if metal_gpu::is_metal_available() && metal_gpu::MetalGpuContext::new().is_err() {
                    // metal_gpu::metal_hash_batch doesn't support dag arg yet?
                    // metal_gpu.rs has hash_batch_with_dag as method on MetalGpuContext.
                    // But metal_hash_batch is a static helper.
                    // I need to check how to call it.
                    // For now, let's just use OpenCL which I know has hash_batch_with_dag exposed.
                }

                // Fallback to OpenCL which works
                match hash_batch_with_dag(header_hash, start_nonce, batch_size, target, &dag) {
                    Ok(Some(result)) => {
                        self.record_gpu_success();
                        Some(result)
                    }
                ...
                #[cfg(feature = "gpu")]
                {
                    if self.gpu_available && self.gpu_enabled_for_session() {
                        ...
                        match hash_batch_with_dag(
                            ...
                            Err(e) => {
                                warn!(
                                    "[Qanhash] OpenCL GPU mining failed: {e}, falling back to CPU"
                                );
                                self.record_gpu_failure();
                            }
                        }
...
pub fn check_pow(hash: &primitive_types::H256, difficulty: f64) -> bool {
    use primitive_types::U256;
    let target = difficulty_to_target(difficulty as u64);

    // STRICT FIX: GPU native output is Little Endian.
    // We MUST convert bytes to U256 as Little Endian for comparison.
    let hash_le = U256::from_little_endian(hash.as_bytes());
    let target_le = U256::from_little_endian(&target);

    // Debug log (optional, remove for production)
    // println!("DEBUG PoW: HashLE={} TargetLE={}", hash_le, target_le);

    hash_le <= target_le
}
pub fn hash_meets_u256_target(hash: &[u8; 32], target: U256) -> bool {
    let mut target_arr = [0u8; 32];
    target.to_big_endian(&mut target_arr);
    hash <= &target_arr
}
