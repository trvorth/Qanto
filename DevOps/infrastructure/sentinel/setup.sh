...
# Phase 43: Expert Shard Enrollment
echo -e "${BLUE}Select Expert Shard Specialization:${NC}"
echo -e "1) Finance (Treasury, Arbitrage)"
echo -e "2) Security (ZK-Verification, Anti-Sybil)"
echo -e "3) General (Model Logic, Inference Hub)"
read -p "Selection (1-3): " SHARD_TYPE

case $SHARD_TYPE in
    1) export SENTINEL_SHARD="FINANCE" ;;
    2) export SENTINEL_SHARD="SECURITY" ;;
    3) export SENTINEL_SHARD="GENERAL" ;;
    *) export SENTINEL_SHARD="GENERAL" ;;
esac

echo -e "${BLUE}Enrolling Sentinel '$NODE_NAME' in '$SENTINEL_SHARD' Shard...${NC}"
# In production: node-cli enroll --key data/node.key --name $NODE_NAME --shard $SENTINEL_SHARD
